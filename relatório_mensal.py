...
def bdays_interval(start_date, end_date):
    start = pd.to_datetime(start_date).year
    end = pd.to_datetime(end_date).year
    feriados = []
    for year in range(start, end+1):
        feriados += Brazil().holidays(year)
    feriados_list = [date for date, name in feriados]
    custom_bday = CustomBusinessDay(holidays=feriados_list)
    business_days = pd.date_range(
        start=start_date, end=end_date, freq=custom_bday)
...
classe_benchmark["class_region"] = classe_benchmark["class"]+"_"+classe_benchmark["region"]
...
def fill_asset_table( start_slide, dict_df, max_rows_len = 18):
    ...
    for index in range(len(dict_df)):
        ...
        while not next_table:
            ...
            df = df.iloc[previous_table_rows_len:] #Seleciona o DataFrame e corta caso já esteja em uso
            
            if previous_table_rows_len!=0:
                table_rows-=1
                remove_row(rentabilidade_ativo_table,rentabilidade_ativo_table.rows[0]) #Remove a primeira linha caso a tabela esteja na metade
            else:
                max_rows_len = max_rows_len-1 if len(table_idx_list)//2 == 1 else max_rows_len+1
            
            table_rows_len = max_rows_len if max_rows_len<len(df) else len(df) #Determina numero de linhas na tabela
            next_table = True if table_rows_len == len(df) else False #Verifica se pode ir para prox tabela
            max_rows_len = max_rows_len-len(df) if max_rows_len-len(df)>=3 else 0 #Verifica se é possível adicionar mais uma tabela
            
            
            for i in range(table_rows_len):
                for j in range(number_of_columns):
                    rentabilidade_ativo_table.cell(i, j+1).text_frame.paragraphs[0].runs[0].text = df.iloc[i,j]
            
            
            for row_idx in range(table_rows_len,table_rows+1):
                remove_row(rentabilidade_ativo_table,rentabilidade_ativo_table.rows[table_rows_len])
            
            if max_rows_len == 0:
                # print("#######")
                for table_idx in range(7): #remove as tabelas nao usadas
                    if table_idx not in table_idx_list:
                        shape = [shape for shape in slide.shapes if shape.name == f"rentabilidade_ativo_{table_idx}"][0]
                        sp = shape._element
                        sp.getparent().remove(sp)
                    
                for idx, elem in enumerate(table_idx_list):
                    shape = [shape for shape in slide.shapes if shape.name == f"rentabilidade_ativo_{elem}"][0]
                    shape.left = 1484154
                    if idx == 0:
                        shape.top = 932400
                    else:
                        prev_shape = [shape for shape in slide.shapes if shape.name == f"rentabilidade_ativo_{elem-1}"][0]
                        shape.top = prev_shape.top + len(prev_shape.table.rows)*Cm(0.79) + Cm(0.25)
    ...
    for table_idx in range(7): #limpa as tabelas nao usadas
        if table_idx not in table_idx_list:
            # print(table_idx)
...
ipca = {}
ipca["mes"] = cmd_historico_acum(username, password, ticker = "IBGE_IPCA15", data_ini = previous_month_last_workday(data_fim).strftime("%d%m%Y")  , data_fim = data_fim_str)
ipca["ano"] = cmd_historico_acum(username, password, ticker = "IBGE_IPCA15", data_ini = previous_month_last_workday(datetime(data_fim.year,1,1)).strftime("%d%m%Y")  , data_fim = data_fim_str)
ipca["12m"] = cmd_historico_acum(username, password, ticker = "IBGE_IPCA15", data_ini = (data_fim+relativedelta(months=-12)).strftime("%d%m%Y")  , data_fim = data_fim_str)
ipca["24m"] = cmd_historico_acum(username, password, ticker = "IBGE_IPCA15", data_ini = (data_fim+relativedelta(months=-24)).strftime("%d%m%Y")  , data_fim = data_fim_str)


cpi = {}
cpi["mes"] = cmd_historico_acum(username, password, ticker = "US:BLS_CPI_forecast", data_ini = previous_month_last_workday(data_fim).strftime("%d%m%Y")  , data_fim = data_fim_str)
cpi["ano"] = cmd_historico_acum(username, password, ticker = "US:BLS_CPI_forecast", data_ini = previous_month_last_workday(datetime(data_fim.year,1,1)).strftime("%d%m%Y")  , data_fim = data_fim_str)
cpi["12m"] = cmd_historico_acum(username, password, ticker = "US:BLS_CPI_forecast", data_ini = (data_fim+relativedelta(months=-12)).strftime("%d%m%Y")  , data_fim = data_fim_str)
cpi["24m"] = cmd_historico_acum(username, password, ticker = "US:BLS_CPI_forecast", data_ini = (data_fim+relativedelta(months=-24)).strftime("%d%m%Y")  , data_fim = data_fim_str)

def tax_diff(taxa1,taxa2):
    is_string = False if not(type(taxa1)==str or type(taxa2)==str) else True
...
def get_cmd_data(portifolio,region="onshore",data_ini = "02012000"):
    ...
    json_e23 = CMD_request("ExtratoCarteira023", portifolio,
                           data_inicio_carteira_str, data_fim_str, first_workday_str)
    ...
    ipca_desde_ini = cmd_historico_acum(username, password, ticker ="IBGE_IPCA15", data_ini = data_inicio_carteira_str, data_fim = data_fim_str)
    
    cpi_desde_ini = cmd_historico_acum(username, password, ticker ="US:BLS_CPI_forecast", data_ini = data_inicio_carteira_str, data_fim = data_fim_str)
...
def perf_attr_fill(order_list, df_PerfAttr, slide):
    ...
    df_PerfAttr['Classe_cat'] = pd.Categorical(
        df_PerfAttr['Classe'], 
    ...
    for i in range(len(df_PerfAttr),7):
        shape = [shape for shape in slide.shapes if shape.name == f"perf_{i}"][0]
        class_ekho = [shape for shape in slide.shapes if shape.name == f"class_{i}"][0]  
    ...
    previous_shape = [shape for shape in slide.shapes if shape.name == "perf_total"][0]
    ...
    for i in range(len(df_PerfAttr)-2,-1,-1):
        previous_shape = [shape for shape in slide.shapes if shape.name == f"perf_{i+1}"][0]
    ...
    class_ekho = [shape for shape in slide.shapes if shape.name == f"class_{0}"][0]
    ...
    label.text_frame.paragraphs[0].runs[0].text = str(int(np.round(df_PerfAttr["Bps no Mês"][0])))
    shape.left = margin
    ...
    for i in range(1, len(df_PerfAttr)):
        ...
        label = [shape for shape in slide.shapes if shape.name == f"label_{i}"][0]
        label.text_frame.paragraphs[0].runs[0].text = str(int(np.round(df_PerfAttr["Bps no Mês"][i])))
    ...
    shape = [shape for shape in slide.shapes if shape.name == "perf_total"][0]
    class_ekho = [shape for shape in slide.shapes if shape.name == "class_total"][0]
    ...
    label.text_frame.paragraphs[0].runs[0].text = str(int(np.round(df_PerfAttr["Bps no Mês"].sum())))
    shape.left = next_margin
    class_ekho.left = next_margin
    label.left = next_margin
...
on_off_errors = []
for client_name in clientes_on_off["name"].unique():
    tentatives = 0
    try:
        print(f"Running {client_name} report. Tentative: {tentatives+1}")
        df_client = clientes_on_off[clientes_on_off["name"]==client_name].reset_index(drop=True)
        portifolio_onshore = df_client[df_client["region"]=="Onshore"]["cmd_portifolio"].iloc[0]
        data_ini_onshore = df_client[df_client["region"]=="Onshore"]["start_date"].iloc[0].strftime("%d%m%Y")
        
        portifolio_offshore = df_client[df_client["region"]=="Offshore"]["cmd_portifolio"].iloc[0]
        data_ini_offshore = df_client[df_client["region"]=="Offshore"]["start_date"].iloc[0].strftime("%d%m%Y")
    
    
        # portifolio_onshore = "1014"  # TODO remover
        # portifolio_offshore = "2014"  # TODO remover
        # client_name="GUSTAVO DORNELLAS TABBAL CHAMATI" #TODO remover
        cmd_onshore = get_cmd_data(portifolio_onshore, region="onshore", data_ini=data_ini_onshore)
        cmd_offshore = get_cmd_data(portifolio_offshore,region="offshore", data_ini=data_ini_offshore)
        ...
        final_onshore["df_final_CarteiraBenchmark"].replace(
            'nd', '-', inplace=True)
        final_onshore["df_final_CarteiraBenchmark"].iloc[4,:] = final_onshore["df_final_CarteiraBenchmark"].iloc[3,:]
        final_onshore["df_final_CarteiraBenchmark"].iloc[3,:] = final_onshore["df_final_CarteiraBenchmark"].iloc[2,:]
        final_onshore["df_final_CarteiraBenchmark"].iloc[2,0] = "Retorno Real"
        final_onshore["df_final_CarteiraBenchmark"].iloc[2,1] = tax_diff(final_onshore["df_final_CarteiraBenchmark"].iloc[1,1],ipca["mes"]) #MÊS
        final_onshore["df_final_CarteiraBenchmark"].iloc[2,2] = tax_diff(final_onshore["df_final_CarteiraBenchmark"].iloc[1,2],ipca["ano"]) #ANO
        final_onshore["df_final_CarteiraBenchmark"].iloc[2,3] = tax_diff(final_onshore["df_final_CarteiraBenchmark"].iloc[1,3],ipca["12m"]) #12M
        final_onshore["df_final_CarteiraBenchmark"].iloc[2,4] = tax_diff(final_onshore["df_final_CarteiraBenchmark"].iloc[1,4],ipca["24m"]) #24M
        final_onshore["df_final_CarteiraBenchmark"].iloc[2,5] = tax_diff(final_onshore["df_final_CarteiraBenchmark"].iloc[1,5],ipca["desde_ini"]) #DESDE O INICIO
        ...
        final_offshore["df_final_CarteiraBenchmark"].iloc[3,:] = final_offshore["df_final_CarteiraBenchmark"].iloc[2,:]
        final_offshore["df_final_CarteiraBenchmark"].iloc[2,0] = "Retorno Real"
        final_offshore["df_final_CarteiraBenchmark"].iloc[2,1] = tax_diff(final_offshore["df_final_CarteiraBenchmark"].iloc[1,1],cpi["mes"]) #MÊS
        final_offshore["df_final_CarteiraBenchmark"].iloc[2,2] = tax_diff(final_offshore["df_final_CarteiraBenchmark"].iloc[1,2],cpi["ano"]) #ANO
        final_offshore["df_final_CarteiraBenchmark"].iloc[2,3] = tax_diff(final_offshore["df_final_CarteiraBenchmark"].iloc[1,3],cpi["12m"]) #12M
        final_offshore["df_final_CarteiraBenchmark"].iloc[2,4] = tax_diff(final_offshore["df_final_CarteiraBenchmark"].iloc[1,4],cpi["24m"]) #24M
        final_offshore["df_final_CarteiraBenchmark"].iloc[2,5] = tax_diff(final_offshore["df_final_CarteiraBenchmark"].iloc[1,5],cpi["desde_ini"]) #DESDE O INICIO
        ...
        union_index = cmd_onshore["df_PL"]["PL"].index.union(cmd_offshore["df_PL"]["PL BRL"].index)
    
        s1_reindexed = cmd_onshore["df_PL"]["PL"].ffill().reindex(union_index, fill_value=0)
        s2_reindexed = cmd_offshore["df_PL"]["PL BRL"].ffill().reindex(union_index, fill_value=0)
        ...
        final_onshore["df_final_RendimentoCarteira"] = final_onshore["df_final_RendimentoCarteira"][[
            "Carteira", "CDI", "PL"]]
        final_onshore["df_final_RendimentoCarteira"].reset_index(inplace=True)
        ...
        try:
            ...
            final_onshore["df_final_PerfAttr"].rename(columns={'Bps no Período': 'Bps no Mês'}, inplace=True)
        ...
        final_onshore["df_final_Volatilidade"] = final_onshore["df_final_Volatilidade"][[
            "Ativo", "Vol. 12m"]]
        # TODO Calcular rendimento anualizado
        ...
        final_onshore["df_final_RetornoNominalAtivo"]["R$ Alocado"] = final_onshore["df_final_RetornoNominalAtivo"]["% Alocado"]*cmd_onshore["df_PL"]["PL"][-1]/100
        ...
        class_index.append(len(final_onshore["df_final_RetornoNominalAtivo"]))
        ...
        for i in range(len(class_index)-1):
            ...
            final_onshore["dfs_class"][categoria_atual] = final_onshore["df_final_RetornoNominalAtivo"].iloc[class_index[i]:class_index[i+1]]
        ...
        final_offshore["df_final_RendimentoCarteira"] = final_offshore["df_final_RendimentoCarteira"][[
            "Carteira", "EFFR", "PL"]]
        final_offshore["df_final_RendimentoCarteira"].reset_index(inplace=True)
    
    
        # SLIDE 8
        final_offshore["df_final_RetornosNominais"] = cmd_offshore["df_RetornosNominais"].copy()
        final_offshore["df_final_RetornosNominais"].iloc[:,1] = final_offshore["df_final_RetornosNominais"].iloc[:,1].apply(lambda x: "%Fed Funds" if "EFFR" in x else x)
        ...
        try:
            ...
            final_offshore["df_final_PerfAttr"] = cmd_offshore["df_PerfAttr"][["Classe", "Bps no Período"]].replace("",np.nan).sort_values(
                by=['Bps no Período'], ascending=False)
            final_offshore["df_final_PerfAttr"].rename(columns={'Bps no Período': 'Bps no Mês'}, inplace=True)
        ...
        final_offshore["df_final_RetornoNominalAtivo"]["$ Alocado"] = final_offshore["df_final_RetornoNominalAtivo"]["% Alocado"]*cmd_offshore["df_PL"]["PL"][-1]/100
    
        final_offshore["df_final_RetornoNominalAtivo"] = final_offshore["df_final_RetornoNominalAtivo"].replace(0, "nd")
        ...
        class_index.append(len(final_offshore["df_final_RetornoNominalAtivo"]))
        final_offshore["dfs_class"] = {}
        for i in range(len(class_index)-1):
            categoria_atual = final_offshore["df_final_RetornoNominalAtivo"].iloc[class_index[i]]['Ativo']
            final_offshore["dfs_class"][categoria_atual] = final_offshore["df_final_RetornoNominalAtivo"].iloc[class_index[i]:class_index[i+1]]
        ...
        prs = Presentation("Novo Template Relatório - ON + OFF.pptx")
    
        # date
        for slide_idx in range(len(prs.slides)):
            slide = prs.slides[slide_idx]
            try:
                date_shape = [shape for shape in slide.shapes if shape.name == "date"][0]
        ...
        for slide_idx in range(len(prs.slides)):
            slide = prs.slides[slide_idx]
            try:
                name_shape = [shape for shape in slide.shapes if shape.name == "name"][0]
        ...
        for i in range(5):
            for j in range(5):
                carteiraBenchmark_on_table.cell(i+2, j+1).text_frame.paragraphs[0].runs[0].text = final_onshore["df_final_CarteiraBenchmark"].iloc[i,j+1]
                carteiraBenchmark_off_table.cell(i+2, j+1).text_frame.paragraphs[0].runs[0].text = final_offshore["df_final_CarteiraBenchmark"].iloc[i,j+1]
                
        # ---define new chart data---
        chart_data = CategoryChartData()
        chart_data.categories = [date for date in PL_total.index]
        chart_data.add_series('PL', tuple(PL_total.ffill().fillna(0).values))
        # ---replace chart data---
        ...
        chart_data.add_series('PL', (final_onshore["PL_final"],final_offshore["PL_final"]))
        # ---replace chart data---
        geografica_graf.replace_data(chart_data)
    
        diversificacao_patrimonio_table.cell(1, 1).text_frame.paragraphs[0].runs[0].text = round_if_numeric(final_onshore["PL_final"], 0, "R$ ")
        diversificacao_patrimonio_table.cell(2, 1).text_frame.paragraphs[0].runs[0].text = round_if_numeric(final_offshore["PL_final"], 0, "R$ ")
        diversificacao_patrimonio_table.cell(3, 1).text_frame.paragraphs[0].runs[0].text = round_if_numeric(final_onshore["PL_final"]+final_offshore["PL_final"], 0, "R$ ")
    
        # slide 3 - Resumo Onshore
        slide = prs.slides[3]
        carteiraBenchmark_on_table = [
            shape for shape in slide.shapes if shape.name == "CarteiraBenchmark"][0].table
        liq_val1 = [shape for shape in slide.shapes if shape.name == "liq_val1"][0].text_frame.paragraphs[0].runs[0]
        liq_val2 = [shape for shape in slide.shapes if shape.name == "liq_val2"][0].text_frame.paragraphs[0].runs[0]
        liq_val3 = [shape for shape in slide.shapes if shape.name == "liq_val3"][0].text_frame.paragraphs[0].runs[0]
        liq_val4 = [shape for shape in slide.shapes if shape.name == "liq_val4"][0].text_frame.paragraphs[0].runs[0]
        ...
        liq1.text_frame.paragraphs[0].runs[0].text = f"{int(np.round(final_onshore['df_final_FluxoResgate'].iloc[0,2]))}%" if final_onshore['df_final_FluxoResgate'].iloc[0,2]>10 else ""
        liq1.width = int(regua.width*final_onshore['df_final_FluxoResgate'].iloc[0,2]/100)
        liq_val2.text = final_onshore["df_final_FluxoResgate"].iloc[1,1]
    
        liq2.text_frame.paragraphs[0].runs[0].text = f"{int(np.round(final_onshore['df_final_FluxoResgate'].iloc[1,2]))}%" if final_onshore['df_final_FluxoResgate'].iloc[1,2]>10 else ""
        liq2.left = liq1.left + liq1.width
        liq2.width = int(regua.width*final_onshore['df_final_FluxoResgate'].iloc[1,2]/100)
        liq_val3.text = final_onshore["df_final_FluxoResgate"].iloc[2,1]
    
        liq3.text_frame.paragraphs[0].runs[0].text = f"{int(np.round(final_onshore['df_final_FluxoResgate'].iloc[2,2]))}%" if final_onshore['df_final_FluxoResgate'].iloc[2,2]>10 else ""
        liq3.left = liq2.left + liq2.width
        liq3.width = int(regua.width*final_onshore['df_final_FluxoResgate'].iloc[2,2]/100)
        liq_val4.text = final_onshore["df_final_FluxoResgate"].iloc[3,1]
    
        liq4.text_frame.paragraphs[0].runs[0].text = f"{int(np.round(final_onshore['df_final_FluxoResgate'].iloc[3,2]))}%" if final_onshore['df_final_FluxoResgate'].iloc[3,2]>10 else ""
        liq4.left = liq3.left + liq3.width
        liq4.width = int(regua.width*final_onshore['df_final_FluxoResgate'].iloc[3,2]/100)
    
        # ---define new chart data---
        chart_data = CategoryChartData()
        chart_data.categories = [date for date in final_onshore["df_final_RendimentoCarteira"]["Data"]]
        chart_data.add_series('Carteira', tuple(final_onshore["df_final_RendimentoCarteira"]["Carteira"].ffill().fillna(0).values))
        chart_data.add_series('CDI', tuple(final_onshore["df_final_RendimentoCarteira"]["CDI"].ffill().fillna(0).values))
        # ---replace chart data---
        ...
        table_retorno_consolidado = [shape for shape in slide.shapes if shape.name == "retorno_consolidado"][0].table
                    
        for row in range(len(final_onshore["df_final_RetornosNominais"])):
            for column in range(16):
                table_retorno_consolidado.cell(2+row, column).text = final_onshore["df_final_RetornosNominais"].iloc[row,column]
                if (row%2):
                    #bench
        ...
        for _ in range(row+3,22):
            remove_row(table_retorno_consolidado, table_retorno_consolidado.rows[row+3])
    
        visao_custodiante = [shape for shape in slide.shapes if shape.name == "visao_custodiante"][0]
        ...
        for i in range(len(total_onshore)-3):
            table_total.cell(0, i+1).text_frame.paragraphs[0].runs[0].text = round_if_numeric(total_onshore[i+2], 0,"R$ ")
        table_total.cell(0, 5).text_frame.paragraphs[0].runs[0].text = round_if_numeric(total_onshore[6], 1,"%", False)
        ...
        for row in range(len(final_onshore["df_final_ResumoPorConta"])):
            for column in range(6):
                table_visao_custodiante.cell(2+row, column).text = final_onshore["df_final_ResumoPorConta"].iloc[row,column+1]
        ...
        for _ in range(row+3,9):
            remove_row(table_visao_custodiante, table_visao_custodiante.rows[row+3])
            
        retorno_consolidado_height = ((len(table_retorno_consolidado.rows)-2)/2)*Cm(1.4) + Cm(1.88)
        visao_custodiante_height = (len(table_visao_custodiante.rows)-2)*Cm(0.8) + Cm(1.8)
        total_height = retorno_consolidado_height + visao_custodiante_height + Cm(0.5)
        slide_height = Cm(19.05)
        [shape for shape in slide.shapes if shape.name == "retorno_consolidado"][0].top = int(slide_height/2 - total_height/2)
        visao_custodiante.top = int([shape for shape in slide.shapes if shape.name == "retorno_consolidado"][0].top + retorno_consolidado_height + Cm(0.5))
        ...
        perf_attr_fill(final_onshore["df_final_AlocacaoClasse"]["Classe"].to_list(), final_onshore["df_final_PerfAttr"], slide)
    
    
        # ---define new chart data---
        chart_data = CategoryChartData()
        chart_data.categories = [date for date in final_onshore["df_final_AlocacaoClasse"]["Classe"]]
        chart_data.add_series('% Carteira', tuple(final_onshore["df_final_AlocacaoClasse"]["% da Carteira"].values))
        # ---replace chart data---
        ...
        for row in range(len(final_offshore["df_final_RetornosNominais"])):
            for column in range(16):
                table_retorno_consolidado.cell(2+row, column).text = final_offshore["df_final_RetornosNominais"].iloc[row,column]
                if (row%2):
                    #bench
                    cell = table_retorno_consolidado.cell(2+row, column)
                    fill = cell.fill
        ...
        for i in range(len(total_offshore)-3):
            table_total.cell(0, i+1).text_frame.paragraphs[0].runs[0].text = round_if_numeric(total_offshore[i+2], 0,"$ ")
        table_total.cell(0, 5).text_frame.paragraphs[0].runs[0].text = round_if_numeric(total_offshore[6], 1,"%", False)
        ...
        for row in range(len(final_offshore["df_final_ResumoPorConta"])):
            for column in range(6):
                table_visao_custodiante.cell(2+row, column).text = final_offshore["df_final_ResumoPorConta"].iloc[row,column+1]
        ...
        perf_attr_fill(final_offshore["df_final_AlocacaoClasse"]["Classe"].to_list(), final_offshore["df_final_PerfAttr"], slide)
    
        # ---define new chart data---
        chart_data = CategoryChartData()
        chart_data.categories = [date for date in final_offshore["df_final_AlocacaoClasse"]["Classe"]]
        chart_data.add_series('% Carteira', tuple(final_offshore["df_final_AlocacaoClasse"]["% da Carteira"].values))
        # ---replace chart data---
        alocacao_classe.replace_data(chart_data)
        ...
        for i in range(5):
            for j in range(5):
                carteiraBenchmark_on_table.cell(i+2, j+1).text_frame.paragraphs[0].runs[0].text = final_offshore["df_final_CarteiraBenchmark"].iloc[i,j+1]
    
        liq_val1.text = final_offshore["df_final_FluxoResgate"].iloc[0,1]
        liq1.text_frame.paragraphs[0].runs[0].text = f"{int(np.round(final_offshore['df_final_FluxoResgate'].iloc[0,2]))}%" if final_offshore['df_final_FluxoResgate'].iloc[0,2]>10 else ""
        liq1.width = int(regua.width*final_offshore['df_final_FluxoResgate'].iloc[0,2]/100)
        liq_val2.text = final_offshore["df_final_FluxoResgate"].iloc[1,1]
    
        liq2.text_frame.paragraphs[0].runs[0].text = f"{int(np.round(final_offshore['df_final_FluxoResgate'].iloc[1,2]))}%" if final_offshore['df_final_FluxoResgate'].iloc[1,2]>10 else ""
        liq2.left = liq1.left + liq1.width
        liq2.width = int(regua.width*final_offshore['df_final_FluxoResgate'].iloc[1,2]/100)
        liq_val3.text = final_offshore["df_final_FluxoResgate"].iloc[2,1]
    
        liq3.text_frame.paragraphs[0].runs[0].text = f"{int(np.round(final_offshore['df_final_FluxoResgate'].iloc[2,2]))}%" if final_offshore['df_final_FluxoResgate'].iloc[2,2]>10 else ""
        liq3.left = liq2.left + liq2.width
        liq3.width = int(regua.width*final_offshore['df_final_FluxoResgate'].iloc[2,2]/100)
        liq_val4.text = final_offshore["df_final_FluxoResgate"].iloc[3,1]
    
        liq4.text_frame.paragraphs[0].runs[0].text = f"{int(np.round(final_offshore['df_final_FluxoResgate'].iloc[3,2]))}%" if final_offshore['df_final_FluxoResgate'].iloc[3,2]>10 else ""
        liq4.left = liq3.left + liq3.width
        liq4.width = int(regua.width*final_offshore['df_final_FluxoResgate'].iloc[3,2]/100)
    
        # ---define new chart data---
        chart_data = CategoryChartData()
        chart_data.categories = [date for date in final_offshore["df_final_RendimentoCarteira"]["Data"]]
        chart_data.add_series('Carteira', tuple(final_offshore["df_final_RendimentoCarteira"]["Carteira"].ffill().fillna(0).values))
        chart_data.add_series('EFFR', tuple(final_offshore["df_final_RendimentoCarteira"]["EFFR"].ffill().fillna(0).values))
        # ---replace chart data---
        carteira_vs_bench.replace_data(chart_data)
    
    
        # Fill onshore tables
        max_slide_onshore = fill_asset_table(7,final_onshore["dfs_class"],18)
    
        # Fill offshore tables
        max_slide_offshore = fill_asset_table(26,final_offshore["dfs_class"],18)
        ...
        prs.save(f"{folder}/{client_name}_{meses[data_fim.month]}2024.pptx")
        print(f"{client_name} report DONE.")
    except:
        if tentatives <=4:
            tentative = tentatives+1
        else:
            on_off_errors.append(client_name)
            pass

# -*- coding: utf-8 -*-
"""
Created on Mon Jul  1 16:42:29 2024
...
for client_name in clientes_on_off["name"].unique():
    ...
    while tentatives < max_tentatives:
        ...
        try:
            print(
                f"\n\nRunning {client_name} report. Tentative: {tentatives+1}")
            df_client = clientes_on_off[clientes_on_off["name"] == client_name].reset_index(
                drop=True)
            portifolio_onshore = df_client[df_client["region"]
                                            == "Onshore"]["cmd_portifolio"].iloc[0]
            data_ini_onshore = df_client[df_client["region"] ==
                                          "Onshore"]["start_date"].iloc[0].strftime("%d%m%Y")

            portifolio_offshore = df_client[df_client["region"]
                                            == "Offshore"]["cmd_portifolio"].iloc[0]
            data_ini_offshore = df_client[df_client["region"] ==
                                          "Offshore"]["start_date"].iloc[0].strftime("%d%m%Y")
            ...
            prs.save(
                f"{folder}/{client_name}_{meses[data_fim.month]}{data_fim.year}.pptx")
            print(f"{client_name} report DONE.")
            break
        except Exception as e:
            tentatives += 1
            print(f"     Tentative {tentatives} failed: {e}")
            # traceback.print_exc()
            if tentatives >= max_tentatives:
                print("Max attempts reached. Passing...")
                on_off_errors.append(client_name)
                break

onshore_errors = []
for client_name in clientes_onshore["name"].unique():
    tentatives = 0
    max_tentatives = 4
    while tentatives < max_tentatives:
        ...
        try:
            print(
                f"\n\nRunning {client_name} ONSHORE report. Tentative: {tentatives+1}")
            df_client = clientes_onshore[clientes_onshore["name"] == client_name].reset_index(
                drop=True)
            portifolio_onshore = df_client[df_client["region"]
                                           == "Onshore"]["cmd_portifolio"].iloc[0]
            data_ini_onshore = df_client[df_client["region"] ==
                    ...
                    for i in range(1, len(final_onshore["dfs_class"][classe])):
                        ...
                        line = pd.DataFrame({'Ativo': benchmark_name,
                                             'Mês': benchmark_diff(ativo["Mês"],
                                                                   accum_return(benchmark, previous_month_last_workday(final_onshore["end_date"]).strftime(
                                                                       "%Y-%m-%d"), final_onshore["end_date"].strftime("%Y-%m-%d"))[-1], benchmark_metric),
                                             'Ano': benchmark_diff(ativo["Ano"], accum_return(benchmark, previous_month_last_workday(datetime(final_onshore["end_date"].year, 1, 10)).strftime(
                                                 "%Y-%m-%d"), final_onshore["end_date"].strftime("%Y-%m-%d"))[-1], benchmark_metric),
                                             '03meses': benchmark_diff(ativo["03meses"],
                                                                       accum_return(benchmark, (final_onshore["end_date"]+relativedelta(months=-3)).strftime(
                                                                           "%Y-%m-%d"), final_onshore["end_date"].strftime("%Y-%m-%d"))[-1], benchmark_metric),
                                             '06meses': benchmark_diff(ativo["06meses"],
                                                                       accum_return(benchmark, (final_onshore["end_date"]+relativedelta(months=-6)).strftime(
                                                                           "%Y-%m-%d"), final_onshore["end_date"].strftime("%Y-%m-%d"))[-1], benchmark_metric),
                                             '12meses': benchmark_diff(ativo["12meses"],
                                                                       accum_return(benchmark, (final_onshore["end_date"]+relativedelta(months=-12)).strftime(
                                                                           "%Y-%m-%d"), final_onshore["end_date"].strftime("%Y-%m-%d"))[-1], benchmark_metric),
                                             '24meses': benchmark_diff(ativo["24meses"],
                                                                       accum_return(benchmark, (final_onshore["end_date"]+relativedelta(months=-24)).strftime(
                                                                           "%Y-%m-%d"), final_onshore["end_date"].strftime("%Y-%m-%d"))[-1], benchmark_metric),
                                             '36meses': benchmark_diff(ativo["36meses"],
                                                                       accum_return(benchmark, (final_onshore["end_date"]+relativedelta(months=-36)).strftime(
                                                                           "%Y-%m-%d"), final_onshore["end_date"].strftime("%Y-%m-%d"))[-1], benchmark_metric),
                                             'Desde aCompra': benchmark_diff(ativo["Desde aCompra"],
                                                                             accum_return(benchmark, ativo["Data Compra"].strftime(
                                                                                 "%Y-%m-%d"), final_onshore["end_date"].strftime("%Y-%m-%d"))[-1], benchmark_metric),
         ...
         ######################## PYTHON PPTX #########################
            ...
            prs = Presentation("templates/Template Relatório - ONSHORE.pptx")
            ...
            prs.save(
                f"{folder}/{client_name}_{meses[data_fim.month]}{data_fim.year}.pptx")
            print(f"{client_name} ONSHORE report DONE.")
            break
        except Exception as e:
            tentatives += 1
            print(f"     Tentative {tentatives} failed: {e}")
            ...
            if tentatives >= max_tentatives:
                print("Max attempts reached. Passing...")
                onshore_errors.append(client_name)
                break



print("\n\nCLIENTES ON + OFF COM ERRO:")
for client in on_off_errors:
    print(client)
